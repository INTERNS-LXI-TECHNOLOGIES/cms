...
public class QualificationResource {
	 ...
	 *         qualificationDTO, or with status 400 (Bad Request) if the
	 *         qualification has already an ID
	...
	public ResponseEntity<QualificationDTO> createQualification(@RequestBody QualificationDTO qualificationDTO)
		...
		if (qualificationDTO.getId() != null) {
			throw new BadRequestAlertException("A new qualification cannot already have an ID", ENTITY_NAME,
					"idexists");
		}
	 ...
	 *         qualificationDTO, or with status 400 (Bad Request) if the
	 *         qualificationDTO is not valid, or with status 500 (Internal Server
	 *         Error) if the qualificationDTO couldn't be updated
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	 ...
	 *         qualificationDTO, or with status 404 (Not Found)
	 */
